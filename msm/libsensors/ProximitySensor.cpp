...
ProximitySensor::ProximitySensor()
    : SensorBase("/dev/cm3602", "proximity"),
      mEnabled(0),
      mInputReader(4),
...
bool ProximitySensor::hasPendingEvents() const {
    D("hasPendingEvents: %s\n", mHasPendingEvent ? "yes" : "no");
...
int ProximitySensor::readEvents(sensors_event_t* data, int count)
{
    ...
    while (count && mInputReader.readEvent(&event)) {
        ...
        if (type == EV_ABS) {
            if (event->code == EVENT_TYPE_PROXIMITY) {
                ...
                D("readEvents: value=%lu distance=%lu\n", event->value, mPendingEvent.distance);
            }
        } else if (type == EV_SYN) {
    ...
    D("readEvents: numEventReceived=%d\n", numEventReceived);
